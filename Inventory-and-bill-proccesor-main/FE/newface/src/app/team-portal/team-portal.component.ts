...
export class TeamPortalComponent implements OnInit, OnDestroy {
  ...
  readonly copy: Record<PortalLanguage, PortalCopy> = {
    ro: {
      ...
      teamHint: 'Vezi numai echipele pe care le coordonezi', notificationHint: 'Alertele de pontaj disponibile pentru rolul tău', back: 'Înapoi la dashboard',
    ...
    en: {
      ...
      teamHint: 'See only the teams you coordinate', notificationHint: 'Attendance alerts available for your role', back: 'Back to dashboard',
    ...
    pa: {
      ...
      teamHint: 'ਸਿਰਫ਼ ਆਪਣੀਆਂ ਟੀਮਾਂ ਵੇਖੋ', notificationHint: 'ਤੁਹਾਡੀ ਭੂਮਿਕਾ ਲਈ ਉਪਲਬਧ ਹਾਜ਼ਰੀ ਸੂਚਨਾਵਾਂ', back: 'ਡੈਸ਼ਬੋਰਡ ਤੇ ਵਾਪਸ',
    ...
    hi: {
      ...
      teamHint: 'केवल अपनी टीम देखें', notificationHint: 'आपकी भूमिका के लिए उपलब्ध उपस्थिति सूचनाएं', back: 'डैशबोर्ड पर वापस',
    ...
    ne: {
      ...
      teamHint: 'आफूले समन्वय गर्ने टोली मात्र हेर्नुहोस्', notificationHint: 'तपाईंको भूमिकाका लागि उपलब्ध हाजिरी सूचनाहरू', back: 'ड्यासबोर्डमा फर्कनुहोस्',
  ...
  notice = '';
  private teamRefreshTimer: ReturnType<typeof setTimeout> | null = null;
  ...
  ngOnDestroy(): void {
    if (this.teamRefreshTimer) clearTimeout(this.teamRefreshTimer);
  }
  ...
  get roleLabel(): string {
    ...
    if (this.dashboard.is_supervisor) labels.push('Supervisor');
  ...
  /** Reîncarcă automat „Echipa mea” când ceasul ajunge la 07:40. */
  private scheduleTeamRefresh(): void {
    if (this.teamRefreshTimer) clearTimeout(this.teamRefreshTimer);
    if (this.view !== 'team') return;
    const now = new Date();
    const nextRefresh = new Date(now);
    nextRefresh.setHours(7, 40, 0, 0);
    if (nextRefresh.getTime() <= now.getTime()) nextRefresh.setDate(nextRefresh.getDate() + 1);
    this.teamRefreshTimer = setTimeout(() => {
      if (this.view === 'team') this.loadCurrentView();
      this.scheduleTeamRefresh();
